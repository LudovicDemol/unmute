...
export default function ScenarioFilters({ scenarios, onChange }: ScenarioFiltersProps) {
  ...
  {/* Search input */}
  <div className="flex-1 relative"> {/* ← relative ici */}
    <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" /> {/* ← absolute + centré */}
    <input
      type="text"
      value={filters.query}
      onChange={(e) => updateFilters({ query: e.target.value })}
      placeholder="Rechercher un cas, un patient, un domaine…"
      className="w-full pl-9 pr-8 py-2.5 text-sm bg-white border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent transition-all"
    /> 
    {filters.query && (
      <button
